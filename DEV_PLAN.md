...
5. **모바일 학습**: PC에서 생성한 퀴즈를 모바일에서도 학습 가능
...
- ✅ SM-2 알고리즘 복습 (무제한)
- ✅ Google 계정 동기화 (무제한)
- ⚠️ **AI 웹페이지 분석: 일일 3회 제한**
- ⚠️ **단어장 다운로드/내보내기 시 광고 표시**

### 광고 기반 수익 (무료 버전)

광고는 다음 3가지 기능 사용 시에만 표시됩니다:

1. **AI 웹페이지 분석**
   - 무료 사용자: 일일 3회 제한
   - 제한 초과 시 광고 시청 후 추가 분석 가능 (예정)

2. **단어장 다운로드**
   - CSV/Anki/Quizlet 형식 내보내기 시 광고 표시
   - 광고 시청 완료 후 다운로드 진행

3. **모바일 퀴즈 생성**
   - 모바일 퀴즈 URL 생성 시 광고 표시 (예정)
   - 광고 시청 후 URL 생성

**광고 형태**: 새 창(팝업) 방식 또는 인터스티셜 광고

### Pro 버전 (향후 계획)
- ✅ **AI 분석 무제한**
- ✅ **광고 완전 제거**
- ✅ 추가 기능 (AI 하이라이트, 고급 통계 등)
- 💲 예상 가격: $2.99/월 또는 $19.99/년
...
### 5. 모바일 퀴즈 (Mobile Quiz)

**기능 설명**:
- PC에서 모바일 퀴즈 URL 생성
- Firebase Realtime Database에 스냅샷 저장
- 생성된 URL을 모바일 브라우저에서 접속하여 학습
- 모바일 최적화 UI (터치 친화적)

**데이터 동기화**:
- 모바일에서 복습한 결과가 PC와 실시간 동기화
- Firebase를 통한 양방향 동기화
...
- **Firebase Realtime Database**: 클라우드 동기화
- **Google Apps Script**: 모바일 퀴즈 서버
...
### Phase 2: Pro 기능 구현 (Q1 2026)
- [ ] Pro 결제 시스템 연동 (Stripe/Paddle)
- [ ] AI 하이라이트 Pro 전용 기능 활성화
- [ ] 광고 시스템 구현 (Google AdSense 또는 자체 광고)
- [ ] 고급 통계 대시보드
- [ ] 학습 목표 설정 및 진행률 추적

### Phase 3: 모바일 앱 개발 (Q2 2026)
